...
export default function StoreSalesPlanEditor({ storeId, plans, currency }: { storeId: string; plans: PlanRow[]; currency: string }) {
  ...
  const now = new Date();
  const { start: fyStart, label: fyLabel } = getFiscalYearRange(now);
  ...
  return (
    <div>
      <p className="text-xs text-slate-400 mb-2">Fiscal year: {fyLabel} (Apr–Mar) — same 12-month cycle used for dealers</p>
